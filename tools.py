...
def split_to_chunk(doc):
    ...
    return chunks
def dataset_creation(chunks):
    if os.path.exists(Path_to_dataset):
        shutil.rmtree(Path_to_dataset)
    dataset = Chroma.from_documents(chunks, OpenAIEmbeddings(), persist_directory=Path_to_dataset)
    dataset.persist()
    print("Dataset created")

if __name__ == "main":
    doc = loader_md("./data")
    chunks = split_to_chunk(doc)
    dataset_creation(chunks)